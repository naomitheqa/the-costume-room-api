...
export const addItem = async function (req, res){
    ...
    try {
        ...
        if (ans.isfound && ans.userType === "ADMIN") {
            const item = await itemController.addItem(name, description, count, group);

            if (item === 0) {
                res.status(400).json(responseController.BadRequest("Data provided for new admin is invalid.",{ name: name, description: description, count: count, group: group }));
            } else if (item === 1) {
                res.status(409).json(responseController.CausingDuplicate("Item with listed name already exists.",{ name: name }));
            } else if (item === 2) {
